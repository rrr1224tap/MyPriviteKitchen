...
const pageDescription = computed(() => {
  if (isMerchantAdmin.value) {
    return `当前食堂房：${merchantId.value || '未识别'}。菜单分类、添个分类、改一下和移除都跟随登录身份。`
  }

  return `当前食堂：${merchantId.value}。菜单分类、添个分类和改一下已接入真实云函数；排序暂不执行真实写入。`
})
const submitLabel = computed(() => {
  ...
  return formMode.value === 'create' ? '食堂记一下' : '改好了'
})
...
async function submitCategoryForm() {
  ...
  try {
    if (formMode.value === 'create') {
      ...
      formSuccessMessage.value = '已经帮你记下，列表已刷新'
      resetForm()
    } else {
  ...
  <section class="page-stack">
    ...
    <GlassCard v-if="isFormOpen">
      <div class="section-heading">
        <div>
          <h2>{{ formTitle }}</h2>
          <p>只会调整分类名称、排序值和状态。厨房身份、分类 ID 和时间字段由后端控制。</p>
        </div>
    ...
    <GlassCard>
      ...
      <EmptyState
        v-else-if="isLoading"
        title="正在加载分类"
        description="正在读取当前食堂房的菜单分类。"
      />

      <EmptyState
        v-else-if="!categories.length"
        title="暂无分类"
        description="当前食堂房还没有可展示分类。可以点击添个分类，先把菜单分组搭起来。"
      >
